...
public final class AtomEntry extends AtomCommonAttributes implements Item {
    ...
    static AtomEntry read(XmlPullParser parser)
            ...
            if (Utils.ATOM_NAMESPACE.equalsIgnoreCase(namespace)) {
                switch (parser.getName()) {
                    ...
                    case AtomFeed.XML_TAG:
                        ...
                        break;
                    default:
                        log.info(TAG, "Unknown tag in Atom entry " + parser.getName());
                ...
                if (Utils.MEDIA_NAMESPACE.equalsIgnoreCase(namespace)) {
                    ...
                    log.info(TAG, "Unknown namespace in Atom item " + parser.getNamespace());
                    Utils.skipTag(parser);
                }
            }
        ...
        if (title == null) {
            log.info(TAG, "No title found for atom entry", new NullPointerException());
            title = new AtomText(null, null, "");
        }
        if (updated == null) {
            log.info(TAG, "No updated found for atom entry, replaced with zero", new NullPointerException());
