...
public final class AtomFeed extends AtomCommonAttributes implements Feed {
    public static final String XML_TAG = "feed";
    ...
    @NonNull
    public static AtomFeed read(XmlPullParser parser, int maxItm) throws IOException,
            ...
            if (Utils.ATOM_NAMESPACE.equalsIgnoreCase(namespace)) {
                ...
                switch (tagName) {
                    ...
                    case "updated":
                        ...
                        break;
                    default:
                        log.info(TAG, "Unknown Atom feed tag " + parser.getName());
                ...
                if (Utils.MEDIA_NAMESPACE.equalsIgnoreCase(namespace)) {
                    ...
                    if (!mediaBuilder.parseTag(parser)) {
                        log.info(TAG, "Unknown mrss tag on feed level");
                        Utils.skipTag(parser);
                    }
                } else {
                    log.info(TAG, "Unknown Atom feed extension " + parser.getNamespace());
                    Utils.skipTag(parser);
                }
            }
        ...
        if (title == null) {
            log.info(TAG, "Missing title tag in atom feed, replacing with empty string");
