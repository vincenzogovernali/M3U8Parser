package it.vincenzogovernali.devilparser.rss.utils;


import it.vincenzogovernali.devilparser.rss.tools.NPTParser;
...
public class Utils {
    ...
    public static final String ITUNES_NAMESPACE = "http://www.itunes.com/dtds/podcast-1.0.dtd";
    ...
    public static Date parseDate(@NonNull String dateString) {
        ...
        if (null == date) {
            log.info(TAG, "Malformed date " + dateString);
        }
        return date;
    }
    ...
    public static Integer parseRFC2326NPT(@NonNull String string) {
        try {
            return (int) new NPTParser(string).parse();
        } catch (ParseException exception) {
            log.info(TAG, "Failed to parse media:rating time", exception);
            return null;
        }
    }
    ...
    public static Integer parseItunesDuration(@NonNull String dateString) {
        for (DateFormat format : itunesDurationFormats) {
            try {
    ...
    public static Integer parseMediaRssTime(@NonNull String time) {
     ...
     */
    public static void finishTag(@NonNull XmlPullParser parser) throws XmlPullParserException, IOException {
     ...
     */
    public static void skipTag(@NonNull XmlPullParser parser) throws XmlPullParserException, IOException {
        if (parser.getEventType() != XmlPullParser.START_TAG) {
    ...
    public static Integer tryParseInt(String string) {
        if (string == null) {
            ...
            try {
                ...
                log.info(TAG, "Error parsing integer value '" + string + '\'', exception);
                return null;
            }
        }
    }
    ...
    @NonNull
    public static Integer nonNullInt(String string) {
        if (string == null) {
            log.info(TAG, "Unexpectedly got null string. -1 returned", new NullPointerException());
            return -1;
        }
        try {
            return Integer.valueOf(string);
        } catch (NumberFormatException exception) {
            log.info(TAG, "Malformed integer string replaced with '-1'", exception);
            return -1;
        }
    }
    ...
    public static String nonNullString(String string) {
        if (string == null) {
            log.info(TAG, "Unexpectedly got null string. Replaced with empty", new NullPointerException());
            return "";
        } else {
            return string;
        }
    ...
    public static URL tryParseUrl(String string) {
        if (string == null) {
            log.info(TAG, "Null value while parsing url", new NullPointerException());
            return null;
        } else {
            try {
                return new URL(string);
            } catch (MalformedURLException exception) {
                log.info(TAG, "Error parsing url value '" + string + '\'', exception);
                return null;
            }
        }
    }
    ...
    @NonNull
    public static URL nonNullUrl(String string) {
        URL result = tryParseUrl(string);
        if (result == null) {
            log.info(TAG, "Malformed URL replaced with 'http://'");
            try {
                result = new URL("http://");
    ...
    public static URI tryParseUri(String string) {
        if (string == null) {
            log.info(TAG, "Null value while parsing uri", new NullPointerException());
            return null;
        } else {
            try {
                return new URI(string);
            } catch (URISyntaxException exception) {
                log.info(TAG, "Error parsing uri value '" + string + '\'', exception);
                return null;
            }
        }
    }
    ...
    @NonNull
    public static URI nonNullUri(String string) {
        URI result = tryParseUri(string);
        if (result == null) {
            log.info(TAG, "Malformed URI replaced with 'http://'");
            try {
                result = new URI("http:///");
