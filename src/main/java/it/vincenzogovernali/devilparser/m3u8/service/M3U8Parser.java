package it.vincenzogovernali.devilparser.m3u8.service;


import it.vincenzogovernali.devilparser.m3u8.model.GenericModel;
import it.vincenzogovernali.devilparser.m3u8.util.RegexUtil;
...
public class M3U8Parser {
    ...
    private List<GenericModel> parseM3U8(String m3u8, String defaultImage) {
        ...
        for (String line : lines) {
            if (title != null && !line.isEmpty() && URLUtil.isValidURL(line)) {
                channels.add(new GenericModel(title, imageUrl, defaultImage, line));
                title = null;
                imageUrl = null;
            } else {
                if (!line.isEmpty()) {
